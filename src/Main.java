...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int i = 1; i <= totalSites; i++) {
                ...
                final boolean isConnected = testConnection(config, myId, i);
                if (!isConnected)
                    return;
            }
            ...
            if (myId == coordinatorId) {
                ...
                // Block until tree is received and processed by ServerThread
                while (!node.isTreeReady()) {
                    Thread.sleep(60000);
                }
                
                Logger.log(myId, "Tree received. Starting menu.");
            }
    ...
    private static boolean testConnection(Config config, int myId, int target) {

        final int MAX_RETRIES = 60;
        final int CONNECTION_TIMEOUT = 5000;  // ms
        int RETRY_DELAY = 5000;  // ms

        final String host = config.getHost(target);
        final int port = config.getPort(target);

        Client client = new Client(config);
        for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            Logger.log(myId, "Connecting to %s:%d  ...".formatted(host, port));
            ...
            if (client.send(ping)) {
                Logger.log(myId, "Ping delivered to " + host + ":" + port);
                return true;
            }

            Logger.log(myId, String.format("Connection failed for " + host + ":" + port + " (attempt %d/%d)", attempt, MAX_RETRIES));
            ...
            try {
                ...
                Logger.log(myId, "Connection retries interrupted.\n" + e);
            }
        }
        return false;
    }
